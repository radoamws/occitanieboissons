...
**Fix (import)** : ajout d'un `$resolveSousFamilleSlug` (même principe que pour les familles, scopé par `famille_id`) dans `ob_get_or_create_sous_famille_by_code()` (`configuration.php`). En cas de collision, le second code obtient un slug suffixé par son code ERP (ex. `alcool-de-riz-42`) au lieu de faire planter l'import.

**Fix (affichage front)** : cette résolution laisse malgré tout deux sous-familles distinctes en base (même libellé, slugs différents), ce qui faisait apparaître 2 cases de filtre identiques "ALCOOL DE RIZ" côté client. Ajout d'un helper `$ob_merge_duplicate_labeled_items()` (`index.php`) qui fusionne, à l'affichage (méga-menu, filtre sidebar "Type"), les sous-familles d'une même famille au libellé strictement identique en une seule case ; sa valeur de filtre regroupe les slugs réels séparés par une virgule (ex. `alcool-de-riz,alcool-de-riz-42`). Le parsing de `filtre_sous_famille` accepte désormais la virgule et explose chaque valeur pour résoudre tous les slugs regroupés vers leurs ids réels : cocher la case unique filtre bien sur les deux codes ERP à la fois.

**À expliquer au client** : ce n'est pas un bug introduit par nous — c'est une incohérence dans les données fournies par le logiciel de gestion (Déclic) : deux codes sous-famille différents affectés au même libellé "ALCOOL DE RIZ" sous la famille ALCOOL. Le script d'import n'était pas assez défensif contre ce cas (contrairement à celui des familles) et une exception SQL non interceptée faisait planter tout le site à chaque page. Corrigé : le site fusionne maintenant ce genre de doublon aussi bien en base (import) qu'à l'affichage (une seule case visible côté client) au lieu de crasher ou d'afficher 2 fois la même chose.

**Note** : si en testant ce filtre aucun produit ne s'affiche, ce n'est pas lié à ce correctif — c'est la règle métier existante qui masque tout produit dont le stock ne permet pas un carton complet (`floor(stock/uv_caisse) > 0`, sauf précommande, voir `ObRenderProduitsGrid()` dans `index.php`). Les produits SAKE utilisés pour le test sont actuellement sous ce seuil.

**Fichiers** : `catalogue/includes/configuration.php` (fonction `ob_get_or_create_sous_famille_by_code`) ; `catalogue/index.php` (helper `$ob_merge_duplicate_labeled_items`, utilisé dans la construction de `sous_familles_all`/`sous_familles_top` et dans `$build_menu_dataset`, et dans le parsing de `filtre_sous_famille`)
...
|------|-------------|-----------|
| 2026-08-19 | Fix crash fatal import : collision de slug entre 2 codes sous-famille ERP portant le même libellé (`uniq_fam_slug`, ex. ALCOOL DE RIZ codes 42/84). Ajout protection anti-collision (suffixe par code) dans `ob_get_or_create_sous_famille_by_code()`, symétrique à celle déjà existante pour les familles. Fusion à l'affichage (méga-menu + filtre sidebar) des sous-familles au libellé identique via `$ob_merge_duplicate_labeled_items()`. Voir §8.10. | `catalogue/includes/configuration.php`, `catalogue/index.php` |
