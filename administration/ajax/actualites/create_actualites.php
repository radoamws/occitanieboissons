<?php
	...
	if(isset($_FILES['image']['name']) && isset($_POST['titre']) && isset($_POST['descr']) && isset($_POST['contenu']) && isset($_POST['tags'])) {
		if(empty($_FILES['image']['name']) || empty($_POST['titre']) || empty($_POST['descr']) || empty($_POST['contenu']) || empty($_POST['tags'])) {
				...
				// GENERATION LIEN IMAGE
					...
					// HIDE
				  	...
				  	// GESTION DES TAGS
						...
						$create_actualites = $bdd->prepare("INSERT INTO ob_actualites (titre, image_short, image, time, contenu, auteur, taglist, hide, descr) VALUES (:titre, :image_short, :image, :time, :contenu, :auteur, :taglist, :hide, :descr)");
						...
						$create_actualites->bindParam(":descr", $_POST['descr']);
