<?php
	...
	if(isset($_POST['id'])) {
		...
		if(isset($_POST['titre']) && isset($_POST['brasseries']) && isset($_POST['contenu']) && isset($_POST['tags'])) {
			if(empty($_POST['titre']) || empty($_POST['contenu']) || empty($_POST['tags']) || empty($_POST['brasseries'])) {
				...
				$modif_news = $bdd->prepare("UPDATE ob_actualites SET titre = :titre, contenu = :contenu, brasseries = :brasseries, taglist = :taglist, hide = '".$hide."', element = :element WHERE id = :id");
				$modif_news->bindParam(":titre", $_POST['titre']);
				$modif_news->bindParam(":brasseries", $_POST['brasseries']);
				$modif_news->bindParam(":taglist", implode(", ", $taglist));
				$modif_news->bindParam(":contenu", $_POST['contenu']);
				$modif_news->bindParam(":element", $_POST['element']);
